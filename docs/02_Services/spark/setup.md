...
> [!NOTE]
> Prefer **`/srv/spark/…`** for durable runtime data. Do **not** keep `spark.eventLog.dir` under `/tmp` — a `/tmp` clean wipes History Server history.

**Switch event logs to `/srv/spark` (on `k8s`):**

```bash
sudo mkdir -p /srv/spark/events && sudo chmod 1777 /srv/spark/events
# spark-defaults.conf:
#   spark.eventLog.enabled=true
#   spark.eventLog.dir=file:/srv/spark/events
#   spark.history.fs.logDirectory=file:/srv/spark/events
sudo /opt/spark/sbin/stop-history-server.sh || true
...
## 4. Control Plane Taint Setup & Workload Verification
...
## 5. Submitting Distributed Spark Jobs
...
## 6. Operations & Web UI Observability
...
* **Lifecycle**: Persistent (24/7) — replays event JSON under `file:/srv/spark/events` (see paths above). Updates near real-time during active jobs while those files remain.
* **Domain Endpoint**: **`https://spark.example.com`** (proxied via Caddy to `192.168.50.96:18080`).


## 7. Cluster & Pod Security Hardening
