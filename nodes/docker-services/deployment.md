# Deployment Strategy for docker-services
...
1. Place secrets on the host:
   ...
   sudo cp nodes/docker-services/services/dbt/.env.example /srv/dbt/.env   # from the clone; edit
   # postgres_password.secret and gcp-creds.json under /srv/dbt/.secrets/
   ```
2. Render profiles:
   ```bash
   sudo /opt/homelab-repo/nodes/docker-services/services/dbt/deploy_app.sh
   ```
