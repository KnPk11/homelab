...
1. Create the env file:
   ...
   sudo cp /opt/homelab-repo/nodes/homelab-95/scripts/ufw.env.example /opt/scripts/Security/ufw.env
...
2. Symlink the tracked script into `/opt/scripts/Security/`, then a short CLI name:
   ```bash
   sudo ln -sfn /opt/homelab-repo/nodes/homelab-95/scripts/ufw.sh /opt/scripts/Security/ufw.sh
