...
*   Deploy tracked scripts from the clone, for example:
    ```bash
    REPO=/opt/homelab-repo/nodes/proxmox-100/scripts
...
```bash
# On proxmox-100, next to the script (or after symlink layout you prefer):
sudo cp /opt/homelab-repo/nodes/proxmox-100/scripts/firewall.env.example \
  /opt/homelab-repo/nodes/proxmox-100/scripts/firewall.env
sudo chmod 600 /opt/homelab-repo/nodes/proxmox-100/scripts/firewall.env
# edit firewall.env — then:
sudo /opt/homelab-repo/nodes/proxmox-100/scripts/firewall.sh
```
