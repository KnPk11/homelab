...
Telegram: `profiles.yaml` sends `http_default` only for SSH brute-force (`ssh-bf`, `ssh-slow-bf`), HTTP admin-interface probing, and Fail2Ban `caddy-authcodes`. Other bans (generic http-probing, user-agents, community ranges, `caddy-sensitivepaths`) still apply, silently. Ban lines are prefixed `🌐`. After deploy:
