...
*   [Monthly audit](../../nodes/ai-tools/services/monthly-audit/deployment.md) — every-other-Sunday 09:00 reminder (no keys). You unlock God Mode and run `--light` or `--deep`; Antigravity (`agy`) or Grok tags the snapshot; digest to Homelab Watch.
*   [WAN saturation](../../nodes/ai-tools/services/wan-saturation/deployment.md) — ai-tools only. Minute poll of `pppoe-out1` via `mikrotik-backup`; Homelab Watch if RX/TX stay near the ISP circuit.
...
sudo /opt/dev/homelab_repo/nodes/ai-tools/services/monthly-audit/deploy.sh
